          ...
          <transition name="fade">
            <el-checkbox-group v-model="checkedPermissions" v-if="showEditOptions">
              <div v-for="(dataPermission, indx) in dataPermissions.data" :key="indx" class="permission-content">
                <el-checkbox :label="dataPermission.id" class="permission-chbx-container" v-if="hasPermissionFile(dataPermissions.type[dataPermission.code])">
...
export default {
  ...
  methods: {
    hasPermissionFile: function (data) {
      if (data === 'folder' && this.returnItem.extension === 'folder') {
        return true
      } else if (data === 'folder_file') {
        return true
      } else if (data === 'file' && this.returnItem.extension !== 'folder') {
        return true
      }
      return false
    },
